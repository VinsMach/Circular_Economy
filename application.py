...
#estraggo i valori della riga corrispondente al vetro pre-cedentemente identificata
valori = arrayCompleto[datiClean[0][0]][1:]

#faccio un ulteriore check e faccio il casting float
valori_float = valori.astype(float)

#per ogni elemento presente nell’array valori finali, fac-cio una trasformazione in grammi e divido per il peso del-la bottiglia 
valori_finali = [round(B *1000000/ pesoBott) for B in va-lori_float]
valori_economici_finali = [(B * valoreBottTonn) for B in valori_float]

#vado a definire un range del dataframe num bottiglie
tabellaNumBott = {'anno': date_finali, 'nume-ro_bottiglie': valori_finali}
...
tabellaCompEcon = {'anno': date_finali, 'Euro guadagna-ti': valori_economici_finali}
...
ax2.set_ylabel("Milioni di Euro Guadagnati", co-lor='green', fontsize=14)
ax2.tick_params(axis="y", labelcolor='green')

l2, = ax2.plot(date_finali ,valori_economici_finali, co-lor='green')
